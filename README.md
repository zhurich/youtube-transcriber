...
Телеграм-бот: присылаете ссылку на YouTube или сам файл (видео, аудио, голосовое,
видеосообщение) — бот извлекает звуковую дорожку, расшифровывает её через
OpenAI Audio API и возвращает текст. Под расшифровкой есть кнопка «Сделать конспект»
(краткие тезисы через chat-модель).

Что внутри:

- `yt-dlp` скачивает с YouTube только аудио, `ffmpeg` жмёт его в mp3 16 кГц моно (64 кбит/с);
...
app/
  ...
  bot.py         хендлеры aiogram, распознавание медиа, очередь задач, отправка результата
...
- **`Telegram не отдаёт боту файлы больше 20 МБ`** — ограничение Bot API, см. раздел
  про файлы из Telegram выше.
