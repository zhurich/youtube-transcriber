...
файлом `.txt`, под ним — обычная кнопка конспекта, которая построит сводку уже по
всей группе. После запуска режим выключается: чтобы собрать следующую группу,
...
app/
  ...
  bot.py         хендлеры aiogram, распознавание медиа, режим группы, очередь задач
